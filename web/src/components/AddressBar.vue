<template>
  <div>
    <div
      id="slider"
      style="margin-top:0px; margin-bottom: 0px; margin-left: 10px; margin-right: 10px; border: 0px; clear:both;"
    >
...
export default {
  ...
  mounted: function () {
    ...
    // we only want sections with ALLOC
    const sections = this.$store.state.sections.filter((section) => {
      for (const flag of section.flags) {
        if (flag.abbrev === 'ALLOC') { return true }
      }
      return false
    })

    this.addressNavBar = new AddressNavBar('#addr-bar-canvas', sections, this.parcels)
