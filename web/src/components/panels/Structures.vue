<template>
  <div>
    ...
    <div class="sidebar-scroll-container">
      <table
        id="structtypes-table"
        class="table table-striped sidebar-table table-sm"
      >
        <thead>
          <tr>
            <th
        ...
        <tbody class="scrollContent">
          <tr v-for="(struct, index) in structTypes">
            <td
              ...
              @click="showEditStructureModal(struct, index)"
            >
              {{ struct.name }}
            </td>
            <td>
    ...
    <b-modal
      ref="createStructModal"
      title="Create Struct Type"
      @ok="createStructure"
    >
    ...
    <b-modal
      ref="editStructModal"
      title="Edit Structure Definition"
      @ok="updateStructure"
    >
      <div class="form-group">
        <table
          id="struct-editor-table"
...
export default {
  ...
  methods: {
    ...
    async createStructure () {
      ...
      const index = _.findIndex(this.$store.state.structTypes,
        { name: this.newStructureName })
      if (index >= 0) {
        const structure = this.$store.state.structTypes[index]
        this.selectedStruct = structure
        this.selectedStructIndex = index
        this.$refs.editStructModal.show()
      }
